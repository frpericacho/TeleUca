                ...
                uploadImage(result.uri).then((resolve:any)=>{
                    ...
                    ref.put(resolve).then(resolve =>{
                        resolve.ref.getDownloadURL().then(url=>{
                            firebase.firestore().collection('users').doc(route.params.User.id).update({
                                avatar_url:url
                            }).then(()=>{
                                setImageUser(url)
                            }).catch((err)=>{
    ...
    return(
        <View style={{flexDirection:'column', height:'100%', justifyContent:'flex-start'}}>
            ...
            <View style={{height:'93%', flex:1, flexDirection: "column"}}>
                <View style={{flex:1}}>
                    <Image
                        source={ImageUser.length!=0 ? {uri:ImageUser} : require('../assets/user.png') }
                ...
                <View style={{flex:2, marginLeft: 20}}>
                    ...
                    <View>
                        {PassBool ? 
                            <View></View>
                        : 
                            <Button onPress={()=>{if(!PassBool){setPassBool(true)} else {setPassBool(false)}}} style={{marginTop:20, marginRight:20}}>
                                Cambiar contraseña
                            </Button>
                        }
                        {PassBool ?
                            <View style={{marginTop:20, marginRight:20}}>
                                ...
                                <Input
                                    secureTextEntry={true} 
                                    ref={input => { textConfirmInput = input }} 
